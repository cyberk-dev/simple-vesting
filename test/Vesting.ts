...
async function deploy() {
  ...
  const { vesting } = await ignition.deploy(VestingModule, {
    parameters: {
      VestingModule: {
        supportedUsdTokens: [usdc.address, usdt.address],
        releaseTimes: [123, 456],
        users: [user1.account.address, user2.account.address],
        userTimeAmounts: [
